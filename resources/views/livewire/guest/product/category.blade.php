    ...
    <div class="w-full sm:w-10/12 px-5 mx-auto my-2 rounded-lg bg-primary-100 dark:bg-transparent">
        <div class="grid grid-cols-3 gap-2 md:gap-4 ">
            <div class="col-span-2 p-5">
                <h2 class="text-2xl text-primary-800 font-bold">{{ __($category->name) }}</h2>
                <p class="text-md text-secondary-700 dark:text-secondary-400"> {{ __($category->description) }}</p>
            </div>
            <img src="{{ $category->thumbnail_url }}" alt="{{ __($category->name) }}"
